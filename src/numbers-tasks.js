...
function getRectangleArea(width, height) {
  const areaOfrestangle = width * height;
  return areaOfrestangle;
}
...
function getCircleCircumference(radius) {
  const ferenceOfcircle = 2 * Math.PI * radius;
  return ferenceOfcircle;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const pointX = (x2 - x1) * (x2 - x1);
  const pointY = (y2 - y1) * (y2 - y1);
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstMulti = (x1 * x1 + y1 * y1) * (x1 * x1 + y1 * y1);
  const secondMulti = (x2 * x2 + y2 * y2) * (x2 * x2 + y2 * y2);
  return Math.acos((x1 * x2 + y1 * y2) / (firstMulti * secondMulti));
}
/**
...
function getSumOfDigits(num) {
  return num
    .toString()
    .split('')
    .reduce((accumulatorNum, currentNum) => accumulatorNum + +currentNum, 0);
}
/**
